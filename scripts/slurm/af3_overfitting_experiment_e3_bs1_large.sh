...
#SBATCH --job-name=af3_overfitting_e3_bs1_large               # job name
...
# Set up WandB run
RUN_ID="1jlc6aic"  # NOTE: Generate a unique ID for each run using `python3 scripts/generate_id.py`
...
srun singularity exec --rocm \
    ...
    "$containerImage" \
    bash -c "python3 -m pip install wandb==0.17.7 && cd /alphafold3-pytorch-lightning-hydra && WANDB_RESUME=allow WANDB_RUN_ID=$RUN_ID python3 alphafold3_pytorch/train.py experiment=af3_overfitting_e3_bs1_large"
