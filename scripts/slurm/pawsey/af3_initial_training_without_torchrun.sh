...
# `train.py` which will then distribute them internally to the processes
# it creates. Importantly, notice that processes are NOT created by srun!
# For what `srun` is concerned, only one task is created, the `train.py` process.

# Define WandB run ID
RUN_ID="z8nbobfe" # NOTE: Generate a unique ID for each run using `python3 scripts/generate_id.py`
